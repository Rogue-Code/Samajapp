  ...
  return (
    <PhoneFrame>
      <div className="flex flex-col items-center justify-center min-h-screen md:min-h-[860px] px-8 text-center">
        ...
        <h2 className="font-bold text-foreground">Facility not found</h2>
        <button onClick={() => navigate({ to: "/facilities" })} className="mt-4 h-10 px-5 rounded-xl bg-primary text-primary-foreground text-sm font-semibold">
          Back to Directory
        </button>
      </div>
    </PhoneFrame>
      ...
      <div className="relative flex flex-col min-h-screen md:min-h-[860px] bg-background">
        ...
        <div className="flex-1 overflow-y-auto pb-28" style={{ scrollbarWidth: "none" }}>
          <div className="px-5 -mt-6 relative">
            <div className="rounded-2xl bg-card border border-border shadow-card p-4">
              <div className="flex items-start gap-2">
                ...
                {f.verified && (
                  <span className="flex items-center gap-1 text-[10px] font-bold uppercase tracking-wider bg-success/10 text-success px-2 py-1 rounded-full">
          ...
          <Section title="Location">
            <a href={mapsUrl} target="_blank" rel="noreferrer" className="block rounded-2xl overflow-hidden border border-border shadow-card">
...
function InfoRow({ icon, label, value, href }: { icon: React.ReactNode; label: string; value: string; href: string }) {
  return (
    <a href={href} className="flex items-center gap-3 py-2 first:pt-0 last:pb-0 border-b last:border-b-0 border-border/60">
